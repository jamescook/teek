...
- `App#clipboard` (`Teek::Clipboard`) — `#set(text)`/`#get`/`#clear`, thin wrappers over Tk's `clipboard` command family. `#get` returns `nil` when the clipboard is empty/unowned instead of raising the `TclError` Tk itself does for that case. Doesn't touch text widgets' own copy/cut/paste — `entry`/`text` already bind `<<Copy>>`/`<<Cut>>`/`<<Paste>>` to the expected platform keys via Tk's own built-in class bindings, confirmed via `event info <<Copy>>` and a real end-to-end test; nothing needed wiring up.
