...
- `Interp#mainloop` no longer holds the GVL for the whole blocking Tk event wait — background Ruby threads could be fully starved and Ctrl-C could hang indefinitely, especially with `thread_timer_ms: 0`.

### Added

- `App.new`/`Teek::UI.app` accept `thread_timer_ms:` to opt into a blocking mode with zero idle wakeups.
