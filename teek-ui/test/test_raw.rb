...
class TestRaw < Minitest::Test
  ...
  def test_raw_runs_at_realize_with_a_working_live_app
    assert_tk_app("ui.raw's block should run at realize, with a real, usable app") do
      require 'teek/ui'
  ...
  def test_raw_does_not_run_before_realize
    assert_tk_app("ui.raw's block should not fire during build, only at realize") do
      require 'teek/ui'
  ...
  def test_raw_op_resolves_a_forward_referenced_sibling_by_name
    assert_tk_app("a raw op declared before a sibling it references should still resolve its realized path") do
      require 'teek/ui'
