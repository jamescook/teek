...
class TestTabs < Minitest::Test
  ...
  def test_tabs_realizes_as_a_working_notebook_with_labeled_pages
    assert_tk_app("ui.tabs should realize as a ttk::notebook with each tab added as a labeled page") do
      require 'teek/ui'
  ...
  def test_tab_content_is_addressable_and_configurable_like_any_other_widget
    assert_tk_app("a widget declared inside a tab should be a normal, addressable, configurable widget") do
      require 'teek/ui'
  ...
  def test_tab_frame_is_not_independently_pack_or_grid_managed
    assert_tk_app("a tab's own frame should be placed only by notebook add, not pack/grid") do
      require 'teek/ui'
  ...
  def test_on_tab_changed_fires_with_the_tabs_name_when_it_has_one
    assert_tk_app("switching to a named tab should fire on_tab_changed with that name") do
      require 'teek/ui'
  ...
  def test_on_tab_changed_fires_with_the_index_when_the_tab_is_unnamed
    assert_tk_app("switching to an unnamed tab should fire on_tab_changed with its zero-based index") do
      require 'teek/ui'
  ...
  def test_on_tab_changed_queued_before_realize_fires_once_wired
    assert_tk_app("on_tab_changed declared before realize should still fire correctly once realized") do
      require 'teek/ui'
  ...
  def test_on_tab_changed_raises_on_a_non_tabs_handle
    assert_tk_app("on_tab_changed should raise a clear error on a non-tabs handle") do
      require 'teek/ui'
  ...
  def test_a_new_tab_can_be_added_at_runtime_via_incremental_realize
    assert_tk_app("session.add should be able to add a whole new tab to an already-realized ui.tabs") do
      require 'teek/ui'
