...
class TestOverlay < Minitest::Test
  ...
  def test_overlay_places_the_widget_at_the_right_anchor
    assert_tk_app("overlay should place its widget in the canvas at the anchor's relx/rely/anchor") do
      require 'teek/ui'
  ...
  def test_two_overlays_position_independently
    assert_tk_app("two overlays on the same canvas should each land at their own anchor") do
      require 'teek/ui'
  ...
  def test_overlay_tracks_a_canvas_resize
    assert_tk_app("an overlay's real on-screen position should scale with the canvas, not stay fixed") do
      require 'teek/ui'
  ...
  def test_overlay_outside_a_canvas_raises
    assert_tk_app("overlay declared outside ui.canvas should raise immediately, not silently do nothing") do
      require 'teek/ui'
  ...
  def test_overlay_with_an_invalid_anchor_raises
    assert_tk_app("overlay given an unrecognized at: should raise, not silently no-op") do
      require 'teek/ui'
  ...
  def test_overlay_requiring_exactly_one_widget_raises_on_zero_or_many
    assert_tk_app("overlay's block must build exactly one widget") do
      require 'teek/ui'
