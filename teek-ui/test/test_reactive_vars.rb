...
class TestReactiveVars < Minitest::Test
  ...
  def test_slider_and_label_stay_in_sync_bidirectionally
    assert_tk_app("a var bound to a slider and a label should keep both in sync") do
      require 'teek/ui'
  ...
  def test_text_box_is_two_way
    assert_tk_app("a var bound to a text_box should sync in both directions") do
      require 'teek/ui'
  ...
  def test_on_change_fires_with_the_coerced_value_regardless_of_which_widget_changed_it
    assert_tk_app("on_change should fire with a coerced Integer, triggered by the bound widget") do
      require 'teek/ui'
  ...
  def test_boolean_var_coerces_correctly_for_a_checkbox
    assert_tk_app("a Boolean var bound to a checkbox should use Tk's 1/0 convention and coerce back to true/false") do
      require 'teek/ui'
