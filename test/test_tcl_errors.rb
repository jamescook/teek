...
class TestTclErrors < Minitest::Test
  ...
  tk_test "a failing eval should still raise with the short Tcl result as the message" do
    err = assert_raises(Teek::TclError) { app.tcl_eval("error {boom}") }
    assert_equal "boom", err.message
  end

  tk_test "a failing nested proc call should populate a multi-frame tcl_backtrace" do
    app.tcl_eval(<<~TCL)
  ...
  tk_test "a failing eval with an explicit errorCode should populate tcl_error_code" do
    err = assert_raises(Teek::TclError) { app.tcl_eval('error "boom" "" {MYAPP BAD_THING}') }

    assert_equal "MYAPP BAD_THING", err.tcl_error_code
  end

  tk_test "a failing eval without an explicit errorCode should report Tcl's default NONE" do
    err = assert_raises(Teek::TclError) { app.tcl_eval("error {boom}") }

    assert_equal "NONE", err.tcl_error_code
  end

  tk_test "a failing tcl_invoke call should populate the same accessors as tcl_eval" do
