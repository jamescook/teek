...
class TestWm < Minitest::Test
  ...
  tk_test "set_window_geometry sets geometry" do
    app.show
    app.update
  ...
  tk_test "set_window_resizable disables resize" do
    app.set_window_resizable(false, false)
    assert_equal [false, false], app.window_resizable
  end

  tk_test "window_resizable returns booleans" do
  ...
  tk_test "on_close with no window: should register on the root window" do
    fired = false
  ...
  tk_test "on_close should not implicitly destroy the window - that's the block's call" do
  ...
  tk_test "an on_close block that calls destroy should actually close the window" do
  ...
  tk_test "destroying a window should release its on_close callback" do
  ...
  tk_test "calling on_close again for the same window should replace, not accumulate" do
  ...
  tk_test "two toplevels should each get their own independent on_close handler" do
  ...
  tk_test "app.wm.set_title/#title should get and set the window title" do
    app.wm.set_title('Hello via wm')
    assert_equal 'Hello via wm', app.wm.title
  end

  tk_test "app.wm.set_title should round-trip a title with an unbalanced brace" do
  ...
  tk_test "app.wm.set_title/#title should accept a window: other than the root" do
  ...
  tk_test "app.wm.set_geometry/#geometry should get and set the window geometry" do
    app.show
    app.update
  ...
  tk_test "app.wm.set_resizable/#resizable should get and set resizable flags" do
    app.wm.set_resizable(false, true)

    assert_equal [false, true], app.wm.resizable
  end

  tk_test "app.wm.deiconify/#withdraw should map/unmap the window" do
    app.wm.deiconify
    app.update
  ...
  # free.
  tk_test "the flat set_window_title should also round-trip an unbalanced brace now" do
    value = 'Title } with brace'
