...
class TestFont < Minitest::Test
  ...
  tk_test "empty string has zero width" do
    w = app.text_width('TkDefaultFont', '')
    assert_equal 0, w
  end

  tk_test "text_width works with font spec" do
  ...
  tk_test "font_metrics returns hash with keys" do
  ...
  tk_test "linespace == ascent + descent" do
    m = app.font_metrics('TkDefaultFont')
    assert_equal m[:ascent] + m[:descent], m[:linespace]
  end

  tk_test "font_metrics works with font spec" do
  ...
  tk_test "measure_chars returns hash" do
  ...
  tk_test "measure_chars respects pixel limit" do
  ...
  tk_test "measure_chars with -1 returns full text" do
    text = 'Hello'
    r = app.measure_chars('TkDefaultFont', text, -1)
    assert_equal text.bytesize, r[:bytes]
  end

  tk_test "measure_chars whole_words option" do
