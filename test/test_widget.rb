...
class TestWidget < Minitest::Test
  ...
  tk_test "auto-naming nests under parent" do
  ...
  tk_test "explicit path is used as-is" do
    frm = app.create_widget('ttk::frame', '.myframe')
    assert_equal '.myframe', frm.path
  end

  tk_test "to_s returns path" do
    btn = app.create_widget('ttk::button', text: 'Hi')
    assert_equal btn.path, btn.to_s
  end

  tk_test "command delegates to app" do
  ...
  tk_test "destroy and exist? work" do
    btn = app.create_widget('ttk::button', text: 'Hi')
  ...
  tk_test "Widget#width/#height should delegate to App#winfo for this widget's path" do
    app.show
  ...
  tk_test "widget works with app.command" do
    btn = app.create_widget('ttk::button', text: 'Hi')
    app.command(:pack, btn, pady: 10)
    assert_equal 'pack', app.tcl_eval("winfo manager #{btn}")
  end

  tk_test "widget tracking works with create_widget" do
    btn = app.create_widget('ttk::button', text: 'Hi')
    app.update
  ...
  tk_test "pack returns self for chaining" do
    btn = app.create_widget('ttk::button', text: 'Hi')
    assert_same btn, btn.pack(pady: 10)
  end

  tk_test "grid returns self for chaining" do
    frm = app.create_widget('ttk::frame')
    frm.pack
    btn = app.create_widget('ttk::button', parent: frm, text: 'Hi')
  ...
  tk_test "bind and unbind delegate to app" do
    btn = app.create_widget('ttk::button', text: 'Hi')
    btn.pack
    bound = false
  ...
  tk_test "Widget#on_close should delegate to App#on_close for this widget's path" do
  ...
  tk_test "configure text with backslash" do
    btn = app.create_widget('ttk::button', text: 'Hi')
  ...
  tk_test "configure text with trailing backslash" do
  ...
  tk_test "destroying a widget should release its -command callback" do
  ...
  tk_test "reconfiguring -command should release the old callback" do
    btn = app.create_widget('ttk::button', text: 'Go', command: proc { })
    baseline = app.interp.callback_ids.length
  ...
  tk_test "multiple Proc-valued options on one widget should be tracked independently" do
    baseline = app.interp.callback_ids.length
  ...
  tk_test "a tracked -command proc should still actually fire" do
    fired = false
